...
# ClaudeShell for Developer ID distribution outside the Mac App Store.
#
# Usage:
...
echo "=== ClaudeShell Developer ID Builder ==="
...
# --- Archive ---
mkdir -p "$DIST_DIR"
ARCHIVE_PATH="$DIST_DIR/ClaudeShell-${VERSION}-${BUILD_NUM}.xcarchive"
rm -rf "$ARCHIVE_PATH"

echo "--- Building archive ---"
xcodebuild \
    -project "$BUILD_DIR/ClaudeShell.xcodeproj" \
    -scheme "$SCHEME" \
...
# --- Export signed .app ---
EXPORT_DIR="$DIST_DIR/ClaudeShell-${VERSION}-${BUILD_NUM}"
...
APP_DIST="$EXPORT_DIR/ClaudeShell.app"
if [ ! -d "$APP_DIST" ]; then
...
if [ "$DO_NOTARIZE" = true ]; then
    NOTARIZE_ZIP="$DIST_DIR/ClaudeShell-${VERSION}-${BUILD_NUM}.zip"
...
# --- Optional DMG ---
if [ "$DO_DMG" = true ]; then
    DMG_PATH="$DIST_DIR/ClaudeShell-${VERSION}-${BUILD_NUM}.dmg"
    echo "--- Building DMG ---"
    rm -f "$DMG_PATH"
    hdiutil create \
        -volname "Claude Shell ${VERSION}" \
        -srcfolder "$APP_DIST" \
        -ov -format UDZO \
        "$DMG_PATH" >/dev/null
...
echo "App:     $APP_DIST"
[ "$DO_DMG" = true ] && echo "DMG:     $DIST_DIR/ClaudeShell-${VERSION}-${BUILD_NUM}.dmg"
